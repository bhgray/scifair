...
{
	global $debug, $aXMLRespTypes, $sXMLDefaultTemplates;
	$sql = "SELECT s.fk_userID, s.currentMoney, u.displayName, u.username, s.fk_lastGamePlayed, u.pk_ID FROM int_user_status AS s, int_users AS u WHERE (s.currentMoney > 0 and s.fk_userID = u.pk_ID) ORDER BY s.currentMoney DESC LIMIT 20";
	$result = mysql_query($sql);
	$error = '';
	...
	while ($leader_row = mysql_fetch_assoc($result)) {
		...
		{
			$displayName = $leader_row['username'];
		}
...
{
	// task=writesurvey&userid=' + user.userID + '&q1ans=' + q1ans + '&q2ans=' + q2ans + '&q3ans=' + q3ans + '&q4ans=' + q4ans
	...
	$sql = sprintf("INSERT INTO `scifair_bhgray_site_aplus_net`.`int_survey` (`pk_surveyID`, `fk_userID`, `timestamp`, `q1`, `q2`, `q3`, `q4`, `q5`) VALUES (NULL, %d, NULL, '%s', '%s', '%s', '%s', '%s');", 
			$userid, mysql_real_escape_string($q1ans), mysql_real_escape_string($q2ans), mysql_real_escape_string($q3ans), mysql_real_escape_string($q4ans), mysql_real_escape_string($q5ans));
	...
	// i'm completely at a loss about this next line... what's wrong?
	//		$output = sprintf($aXMLRespTypes['writesurvey'], $error, mysql_affected_rows(), $userid);
	$output = "<writesurvey><error>" . $error . "</error><rows>" . mysql_affected_rows() . "</rows><userid>" . $userid . "</userid></writesurvey>";
	return $output;	
}
...
//	$sql = 'UPDATE `scifair_bhgray_site_aplus_net`.`int_users` SET `last_login` = NOW( ), `surveyTaken` = ' . 1 . ' WHERE `int_users`.`pk_ID` = ' . (int)$userid . ' LIMIT 1;'; 
	...
	$output = "<marksurveycomplete><error>" . $error . "</error><rows>" . mysql_affected_rows() . "</rows><userid>" . $userid . "</userid></marksurveycomplete>";
	return $output;	
}
