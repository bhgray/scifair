...
function getlogin() 
{
	...
	$output = sprintf($aXMLRespTypes['getlogin'], $error, $name, $sdid, $auth, $lastlogin, $message, $permitted);
	return $output;
}
...
function createuserStatus()
{
	...
	$sql = 'INSERT INTO `scifair_bhgray_site_aplus_net`.`int_user_status` (`fk_userID`, `fk_lastGamePlayed`, `currentMoney`) VALUES (\'' . mysql_real_escape_string($id) . '\', \'0\', \'100\')';
	$result = mysql_query($sql);
	$error = '';
	if (!$result) {
		$error = mysql_error();
