...
//		window.alert((new XMLSerializer()).serializeToString(o.responseXML));
		...
		for (i = 0; i < rows.length; i++) {
			...
			leaderBoard.push(new Leader(name, money, gamesPlayed, '0'));
		}
...
{
	return 'Grand Total:  ' + user_status.money + '<br />Games Remaining:  ' + (user.gamesPermitted - user_status.lastGamePlayed);
}
...
{
	var result = 0;
//	var resultString = 'The winner was:  1 : ' + winner + '.  ';
	var resultString = '';
	if (odds == winner)
	{
		result = bet * odds;
